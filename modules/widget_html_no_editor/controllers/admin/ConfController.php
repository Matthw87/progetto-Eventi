...
class ConfController extends Controller{
	...
	function display(){
		...
		if( $this->isSubmitted()){
			...
			$array = $this->checkDataForm('widget_html_no_editor_conf',$formdata);
			if( $array[0] == 'ok'){
				unset($array[0]);
		...
		$dataform = $this->getDataForm('widget_html_no_editor_conf',$dati);
			
		$this->setVar('dataform',$dataform);
		$this->output('@widget_html_no_editor/admin/impostazioni.htm');
	}
